...
function change(id) {
    ...
    for (let i = 0; i < 4; i++) {
        if (i !== values[0] ) {
            if (matrix[i][values[1]] === true) {
                matrix_square[i][values[1]].style.backgroundColor = 'pink';
                matrix[i][values[1]] = false;
            } else {
                matrix_square[i][values[1]].style.backgroundColor  = 'white';
    ...
    for (let j = 0; j < 4; j++) {
        if (matrix[values[0]][j] === true) {
            matrix_square[values[0]][j].style.backgroundColor  = 'pink';
            matrix[values[0]][j] = false;
        }
        else {
            matrix_square[values[0]][j].style.backgroundColor  = 'white';
